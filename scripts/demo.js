...
class AIAgentDemo {
  ...
  async demonstrateConversation() {
    ...
    for (const [index, message] of conversations.entries()) {
      ...
      try {
        ...
        console.log(`   - 工作流路径: ${response.metadata.workflowPath.join(' -> ')}`);
        console.log(`   - 意图识别: ${response.metadata.intent} (置信度: ${response.metadata.confidence})`);
  ...
  async demonstrateStats() {
    ...
    try {
      ...
      // 显示工作流统计
      if (stats.workflowStats) {
        console.log('   - 工作流统计:');
        console.log(`     * 节点数: ${stats.workflowStats.nodes.length}`);
        console.log(`     * 边数: ${stats.workflowStats.edges.length}`);
        console.log(`     * 描述: ${stats.workflowStats.description}`);
      }
