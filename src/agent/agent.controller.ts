...
export class AgentController {
  ...
  async getStats(): Promise<any> {
    ...
    const documents = await this.knowledgeBase.getAllDocuments();
    const schemas = await this.text2sql.getTableSchemas();
    const workflowStats = this.aiAgentService.getWorkflowStats();
  ...
  @Get("workflow/stats")
  @ApiOperation({ summary: "获取工作流统计信息" })
  @ApiResponse({ status: 200, description: "获取成功" })
  async getWorkflowStats(): Promise<any> {
    return this.aiAgentService.getWorkflowStats();
  }
