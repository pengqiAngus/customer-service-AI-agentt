...
export class ConversationMemory {
  ...
  async addMessage(
    ...
    if (role === 'user') {
      ...
      if (lastUserMessage) {
        await memory.saveContext({ input: lastUserMessage.content }, { output: content });
      }
    }
  ...
  private async loadConversationHistory(
    ...
    if (fs.existsSync(historyPath)) {
      try {
        ...
        this.logger.log(`为会话 ${sessionId} 加载了 ${history.length} 条历史消息`);
      } catch (error) {
  ...
  private extractTopics(messages: string[]): string[] {
    ...
    // 简单的关键词提取
    const keywords = ['产品', '订单', '退款', '保修', '技术支持', '价格', '配送', '客服'];
  ...
  private extractIntent(messages: string[]): string {
    ...
    if (allText.includes('技术') || allText.includes('故障')) return 'technical_support';
